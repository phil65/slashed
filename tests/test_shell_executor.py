...
class EchoCommand(SlashedCommand):
    ...
    async def execute_command(self, ctx: CommandContext[None], *args: str) -> CommandResult:
...
class GrepCommand(SlashedCommand):
    ...
    async def execute_command(self, ctx: CommandContext[None], pattern: str) -> CommandResult:
...
class HeadCommand(SlashedCommand):
    ...
    async def execute_command(self, ctx: CommandContext[None], n: int = 10) -> CommandResult:
        lines = ctx.stdin.splitlines()
        result = lines[:n]
        return CommandResult(stdout="\n".join(result) + "\n" if result else "")
...
async def test_echo(store: CommandStore) -> None:
...
async def test_unknown_command(store: CommandStore) -> None:
...
async def test_simple_pipe(store: CommandStore) -> None:
...
async def test_multi_pipe(store: CommandStore) -> None:
    """Test multiple pipes."""
...
async def test_pipe_with_filtering(store: CommandStore) -> None:
...
async def test_pipe_head(store: CommandStore) -> None:
...
async def test_and_success(store: CommandStore) -> None:
...
async def test_and_failure(store: CommandStore) -> None:
...
async def test_or_success(store: CommandStore) -> None:
...
async def test_or_failure(store: CommandStore) -> None:
...
async def test_sequential(store: CommandStore) -> None:
...
async def test_complex_chain(store: CommandStore) -> None:
